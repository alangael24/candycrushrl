...
class CandyCrush(nn.Module):
    def __init__(self, env, hidden_size=256, board_channels=32, meta_hidden_size=128, **kwargs):
        ...
        self.board_encoder = nn.Sequential(
            ...
            nn.GELU(),
            layer_init(nn.Conv2d(board_channels, board_channels * 2, kernel_size=3, padding=1, stride=2)),
            nn.GELU(),
            layer_init(nn.Conv2d(board_channels * 2, board_channels * 2, kernel_size=3, padding=1)),
            nn.GELU(),
            nn.Flatten(),
        )
        board_out_dim = (board_channels * 2) * ((self.board_size + 1) // 2) * ((self.board_size + 1) // 2)

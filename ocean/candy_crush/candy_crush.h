...
#define GOAL_AUTO -1
#define CANDY_BOARD_SIZE 8
#define CANDY_NUM_CANDIES 6
#define CANDY_GOAL_SLOTS (CANDY_NUM_CANDIES + GOAL_EXTRA_SLOTS)
#define CANDY_ACTION_COUNT (CANDY_BOARD_SIZE * CANDY_BOARD_SIZE * 4)
#define CANDY_OBS_LAYERS (SPECIAL_LAYERS * CANDY_NUM_CANDIES + 4)
#define CANDY_SCALAR_FEATURES (CANDY_GOAL_SLOTS * 3 + 2)
#define CANDY_BOARD_FEATURES (CANDY_BOARD_SIZE * CANDY_BOARD_SIZE * CANDY_OBS_LAYERS)
#define CANDY_OBS_FEATURES (CANDY_BOARD_FEATURES + CANDY_SCALAR_FEATURES)
#define CANDY_OBS_TOTAL_SIZE (CANDY_OBS_FEATURES + CANDY_ACTION_COUNT)
...
static void init_env(CandyCrush* env) {
    ...
    if (env->board_size != CANDY_BOARD_SIZE) { fprintf(stderr, "candy_crush: board_size must be %d in the 4.0 static build\n", CANDY_BOARD_SIZE); exit(1); }
    if (env->num_candies != CANDY_NUM_CANDIES) { fprintf(stderr, "candy_crush: num_candies must be %d in the 4.0 static build\n", CANDY_NUM_CANDIES); exit(1); }
    ...
    if (!env->has_goal_vector && env->task_distribution_mode == 0 && env->level_id < 0 && env->curriculum_mode == 0) {
        fprintf(stderr, "candy_crush: provide goal_* values or enable task_distribution/curriculum/level profiles\n");
        exit(1);
    }
    clear_goal_vector(env->goal_remaining);
    ...
    env->frontier_episodes = 0;
    env->frontier_wins = 0;
    if (env->rng_seed == 0) env->rng_seed = (uint64_t)env->rng + 1;
    ensure_rng_seeded(env);

...
def load_base_args(env_name: str, config_file: Path | None = None) -> dict[str, Any]:
    argv = sys.argv[:]
    try:
        sys.argv = [sys.argv[0]]
        if config_file is None:
            return pufferl.load_config(env_name)
        return pufferl.load_config_file(str(config_file))
    finally:
        sys.argv = argv
...
def apply_a0(
    ...
    total_timesteps: int,
    data_dir: str,
    env_num_envs: int | None = None,
    vec_num_envs: int | None = None,
) -> dict[str, Any]:
    ...
    if env_num_envs is not None:
        args["env"]["num_envs"] = env_num_envs
    if vec_num_envs is not None:
        args["vec"]["num_envs"] = vec_num_envs
...
def run_guardrail(
    ...
    base_args = load_base_args(env_name, config_file=config_file)
    ...
    return {
        "env_name": env_name,
        "config_file": None if config_file is None else str(config_file),
        "device": device,
        "total_timesteps": total_timesteps,
        "seeds": seeds,
...
def main() -> None:
    ...
    record.add_argument("--config-file", type=Path, default=DEFAULT_A0_CONFIG)
    ...
    record.add_argument("--env-num-envs", type=int, default=None)
    record.add_argument("--vec-num-envs", type=int, default=None)
    ...
    check.add_argument("--config-file", type=Path, default=DEFAULT_A0_CONFIG)
    ...
    check.add_argument("--env-num-envs", type=int, default=None)
    check.add_argument("--vec-num-envs", type=int, default=None)
    ...
    report = run_guardrail(
        ...
        env_num_envs=args.env_num_envs,
        vec_num_envs=args.vec_num_envs,
    )
